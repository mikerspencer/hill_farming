...
ggplot(y, aes(prop2001, A_agri.x)) +
   ...
   geom_text(aes(label=ifelse(!between(diff2001, -1000, 1000), 
                              as.character(PARName), "")),
             hjust=0, vjust=0) +
   labs(title="Those aged 16-74 in agriculture, forestry or fishing employment by parish",
        subtitle="2001",
        x="Aggregated to agricultural parishes",
        y="Downloaded for civic parishes") +
...
The working in agriculture population for aggregated parishes is `r sum(census$pop_working_land_2011)` people and for civic parishes `r sum(civ_parish_2011$A_agri/100 * civ_parish_2011$all, na.rm=T)` people.
compare them to the census output area value, which is `r sum(census_OA$pop_working_land_2011)`.

compare civic and ag parish sizes.
