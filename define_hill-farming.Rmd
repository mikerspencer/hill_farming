...
```{r common_grazing}
cg = read_csv("~/Cloud/Michael/SRUC/hill_farms/data/spatial-processed/common_grazing.csv") %>% 
   select(-Name)

parishes = parishes %>% 
   left_join(cg) %>% 
   mutate(common_grazing=replace(common_grazing, is.na(common_grazing), 0))
...
parishes %>% 
   ggplot(aes(long, lat, fill=common_grazing, group=group)) +
   geom_polygon() +
   coord_equal() +
   scale_fill_viridis() +
   labs(fill="Common\nGrazing\nProportion") +
...
restrictions %>% 
   select(elev__median, slope__median, landscape_class_prop, land_cap_ag_prop, land_cap_forest_prop, wildland_prop, rural_prop, peat_prop, NNR_prop) %>% 
   pairs()
```
...
```{r build}
x = restrictions %>% 
   select(elev__median, slope__median, landscape_class_prop, land_cap_ag_prop, land_cap_forest_prop, wildland_prop, rural_prop, NNR_prop, peat_prop)
...
```{r run}
fit = kmeans(x, 2) # 2 cluster solution

# append cluster assignment
restrictions = data.frame(restrictions, cluster=fit$cluster)
```

```{r map}
parishes = parishes %>% 
   left_join(select(restrictions, PARCode, cluster))

parishes %>% 
   ggplot(aes(long, lat, fill=as.character(cluster), group=group)) +
   geom_polygon() +
   coord_equal() +
