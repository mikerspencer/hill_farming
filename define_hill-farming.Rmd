...
```{r SNH_peat_areas}
data.frame(potential_peat_areas=c("Blanket bog/peat. veg.", "Water", "Montane veg.", "Cliffs", "Dubh lochans", "Undiff. heather moor", "Other peat", "Wet heather moor", "Dry heather moor", "Quarries", "Wetlands", "Dune lands", "Bings (area)", "Smooth grass/rushes", "Undiff. Nardus/Molinia", "Snow cover", "Industrial peat", "Ski tows", "Rhododendron"))
```
...
restrictions %>% 
   select(elev__median, slope__median, landscape_class_prop, land_cap_ag_prop, land_cap_forest_prop, wildland_prop, rural_prop) %>% 
   pairs()
```
...
parishes %>% 
   ggplot(aes(long, lat, fill=cluster, group=group)) +
   geom_polygon() +
   coord_equal() +
   scale_fill_viridis() +
